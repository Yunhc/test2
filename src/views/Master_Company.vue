    ...
    <div class="window-search-1">
      <form class="d-flex" :style="{height:'37px'}" >
        <div class="input-group mb-3" :style="{ margin:'5px 5px 0px 5px'}">
          <span class="input-group-text btn-sm" id="basic-addon1">{{lblCompanyID}}</span>
          <input type="text" class="form-control btn-sm" placeholder="CompanyID" aria-label="CompanyID" aria-describedby="basic-addon1" v-model="company_param.companyid">
        </div>

        <div class="input-group mb-3" :style="{ margin:'5px 5px 0px 5px'}">
          <span class="input-group-text btn-sm" id="basic-addon1">{{lblCompanyName}}</span>
          <input type="text" class="form-control btn-sm" placeholder="CompanyName" aria-label="CompanyName" aria-describedby="basic-addon1" v-model="company_param.companyname">
        </div>
        ...
        <div class="input-group mb-3" :style="{ margin:'5px 5px 0px 0px'}">
          ...
          <select class="form-select btn-sm" aria-label="Default select example" v-model="company_param.useflag">
...
<script>
  ...
  export default {
    ...
    setup(props, {emit}){
      ...
      //화면 언어 설정==============================================================================//
      let lang = ref(language.master_company);

      let lblCompanyID = ref("회사코드");
      let lblCompanyName = ref("회사명");
      ...
      let company_param = reactive({companyid:"", companyname:"", useflag:""});
      ...
      let columnDefs= reactive([
          ...
          {headerName: '회사코드', field: 'bukrs', width: 100, sortable: true, pinned: 'left',
            cellRenderer: BoldRenderer,
          },
          {headerName: '회사명', field: 'comname', width: 120, pinned: 'left'},
      ...
      function searchClick_post() {
        let urlPost = url.value + '/dw/master/company/search';

         console.log("[company_param.company_param]", company_param.companyid);
         console.log("[company_param.companyname]", company_param.companyname);
         console.log("[company_param.useflag]", company_param.useflag);

        $axios.post(urlPost, {
          i_lang: "KR",
          i_bukrs: company_param.companyid,
          i_companynm: company_param.companyname,
          i_useflag: company_param.useflag,
        })
        .then((res) => {
      ...
      function closeClick(){
        console.log("component_close = ", "master_company");
        emit("component_close", "master_company");
      }

      function setLanguage(){
        console.log("lang = ", lang.value);
        lblCompanyID.value = lang.value['lblCompanyID'][store.state.setup.language];
        lblCompanyName.value = lang.value['lblCompanyName'][store.state.setup.language];
      ...
      function createIndexedDB(){
        if(!window.indexedDB){
          ...
          request.onupgradeneeded = function(event){
            ...
            //   CompanyData.forEach(function(user) {
      ...
      function selectIndexedDB(){
        ...
        request.onsuccess = function(event){
          ...
          var request = objectStore.get(company_param.companyid);
          request.onerror = function(){
            console.log("select all fail");
          }
      ...
      function deleteIndexedDB(){
        ...
        request.onsuccess = function(event){
          ...
          var objectStore = transaction.objectStore('silsa');
          var request = objectStore.delete(company_param.companyid);
          request.onsuccess = function(event){
            console.log(event.target.result);
          }
