    ...
    <div class="pop-up-window-search-5">
      <div class="input-group mb-3" :style="{ margin:'3px 5px 0px 0px'}">
        <span class="input-group-text btn-sm" id="basic-addon1"
          :style="{width:'80px', display:'inline-block', 'text-align':'right'}">PO 일자
        </span>
        <v-date-picker
          mode="date"
          v-model="date"
      ...
      <div class="input-group mb-3" :style="{ margin:'-15px 0px 0px 0px'}">
        <span class="input-group-text btn-sm" id="basic-addon1"
          :style="{width:'80px', display:'inline-block', 'text-align':'right'}">PO No
        </span>
        <label type="text" autocomplete="off" class="form-control btn-sm" placeholder="PO No"
            aria-label="PO No" aria-describedby="basic-addon1"
            :style="{'text-align':'left'}">
            {{lblPONo}}
        </label>
      </div>
...
export default {
  ...
  setup(props, {emit}){
    ...
    let columnDefs= reactive([
      ...
      {headerName: 'PO No', field: 'vbeln', width: 15, cellStyle: {textAlign: "center"}, sortable: true, pinned: 'left'},
    ...
    var gridOptions = {
      ...
      onRowClicked : function(event){
          ...
          lblPONo.value = selectedRow.vbeln;
    ...
    onMounted(() => {
      console.log("[Good Receipt Po Search] = ", "onMounted--");
      window.addEventListener('resize', handleResize);
      // txtPO.value.focus();

      if (isRange.value == true){
        console.log("[Good Receipt PO Search] = onMounted -- start --", date.value.start);
        console.log("[Good Receipt PO Search] = onMounted -- end --", date.value.end);
      }
      else{
        date.value = new Date();
        console.log("[Good Receipt PO Search] = onMounted -- date --", date.value);
      }
    });
    ...
    function displayClick(){
      ...
      // console.log("displayClick_PO -- data -- ", formatDate(date, "YYYYMMDD"));

      if (isRange.value == true){
        console.log("[Good Receipt Po Search] = onMounted -- start --", formatDate(date.value.start, "YYYYMMDD"));
        console.log("[Good Receipt Po Search] = onMounted -- end --", formatDate(date.value.end, "YYYYMMDD"));
      }
      else{
        console.log("[Good Receipt Po Search] = onMounted -- date --", formatDate(date.value, "YYYYMMDD"));
      }
      ...
      .then((res) => {
        ...
        if(res.data.length > 0){
          ...
          if (res.data[0].code == "NG"){
            ...
            // lblPONo.value = res.data[0].vbeln;
    ...
    function POselectClick(){
      if (lblPONo.value){
        emit("POselectClick", lblPONo.value);
      }
      else {
        alert("Please search PO information and select one PO with tapping a row first")
      }
      // emit("POselectClick");
    }
