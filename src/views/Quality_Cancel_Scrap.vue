    ...
    <div class="window-search-5">
      ...
      <div class="input-group mb-3" :style="{ margin:'-15px 0px 0px 0px'}">
        ...
        <label type="text" autocomplete="off" class="input-group-text btn-sm" placeholder="Box"
            ...
            :style="{border: 'none', margin:'0px 20px 0px -15px', background:'white', width: '50px', 'text-align':'left'}">
            {{lblUnit}}
        </label>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    setup(props,{emit}){
      ...
      //화면 언어 설정==============================================================================//
      let lang = ref(language.quality_cancel_scrap);
      ...
      var gridOptions = {
        ...
        onGridReady: function(event) {
          console.log("[Quality_Cancel Scrap] = ", "onGridReady");
      ...
      onUnmounted(() =>{
        console.log("[Quality_Cancel Scrap] = onUnmounted -- ");
      ...
      async function fn_Search(){
        ...
        .then((res) => {
          ...
          if(res.data.length > 0) {
            ...
            lblUnit.value = res.data[0].meins;
      ...
      async function yesClick() {
        ...
        if (strCalltype.value == "send"){
          ...
          emit("component_close", "quality_cancel_scrap");
        }
      ...
      function sendClick(){
        ...
        if (rowCount > 0){  //미전송 데이터가 있는 경우만 사용자 확인후 처리한다.           
          popupTitle.value ="Quality_Cancel Scrap";
      ...
      function deleteClick(){
        ...
        if (rowCount > 0){  //스캔한 바코드가 있는 경우만 사용자 확인후 처리한다.
          ...
          if (selectedData.length > 0) {
            popupTitle.value ="Quality_Cancel_Scrap";
